...
- Premium pack foundation templates include `21-Day Reset Pack` with `packId = 21_day_reset`, `Beginner Consistency Plan` with `packId = beginner_consistency`, `30-Day Discipline Challenge` with `packId = discipline_30`, and `75-Day Tribe Mode` with `packId = tribe_mode_75`.
