...
- Admin profile surfaces can save and review Referral Reward Handoff Audit Review Records under `referralRewardHandoffAuditReviews` from aggregate referral claim, tier, referral-join, and campaign-reach evidence without granting Pro, writing entitlements, unlocking challenge packs, creating payouts, purchases, discounts, affiliate rewards, referral state, link-open counts, fulfillment claims, tracking pixels, auto-messages, scraped/stored replies, or pressure.
- Admin profile surfaces show a Referral Reward Handoff Audit Decision Reply Kit with copy-ready approved/waiting/not-ready/declined manual replies for referral reward handoff audit records.
...
Referral Reward Handoff Audit Review Records are manual admin evidence only across Web, iOS, and Android. They save to `referralRewardHandoffAuditReviews` with `manualReviewOnly: true`, `grantsPro: false`, `writesEntitlements: false`, `createsPayouts: false`, `createsPurchases: false`, `createsDiscounts: false`, `createsAffiliateRewards: false`, `writesReferralState: false`, `countsLinkOpens: false`, `claimsFulfillment: false`, `hasTrackingPixels: false`, `autoMessagesUsers: false`, `scrapesMessages: false`, `storesReplies: false`, and `pressuresMembers: false`; review decisions do not grant Pro, write entitlements, unlock challenge packs, create payouts, purchases, discounts, affiliate rewards, referral-state writes, link-open counts, fulfillment claims, tracking, auto-messages, scraped/stored replies, or pressure.

Referral Reward Handoff Audit Decision Reply Kit is copy-only/manual UI and must not grant Pro, write entitlements, unlock challenge packs, create discounts, create payouts, create purchases, create affiliate rewards, write referral state, count link opens, claim fulfillment, promise outcomes, imply medical results, scrape/store DMs or replies, add tracking pixels, auto-message users, or pressure members.
...
| `creatorChallengeTemplateDrafts/{draftId}.candidateChallengeName` | string | Review-only reusable hosted challenge template title, usually seeded from the next hosted challenge. |
| `creatorChallengeTemplateDrafts/{draftId}.status` | string | Manual review state. Initial value is `open`; admins can set `approved`, `published`, `waiting`, `not_ready`, or `declined`. |
| `creatorChallengeTemplateDrafts/{draftId}.reviewNote` | string, optional | Admin-only manual review note for reusable format, rules, prompts, safety, moderation, privacy, and support boundaries. |
| `creatorChallengeTemplates/{templateId}.draftId` | string | Source creator template draft used for admin-reviewed publication. |
| `creatorChallengeTemplates/{templateId}.status` | string | Published catalog state. Initial public value is `published`. |
| `creatorChallengeTemplates/{templateId}.isPublic` | boolean | Must be `true` for signed-in members to read the reviewed free creator template. |
| `creatorChallengeTemplates/{templateId}.isPremium` | boolean | Must be `false` for this free-first creator template catalog slice. |
| `creatorChallengeTemplates/{templateId}.publishedBy` | string | Admin display name/email recorded when a draft is published. |
| `challenges/{challengeId}.templateSource` | string, optional | Set to `creatorChallengeTemplates` when a member creates from a published creator template. |
| `challenges/{challengeId}.creatorTemplateId` | string, optional | Published creator template id used to seed the challenge. |
| `challenges/{challengeId}.creatorTemplateDraftId` | string, optional | Source reviewed draft id for audit lineage. |
| `challenges/{challengeId}.creatorTemplateOwnerUid` | string, optional | Creator uid from the published template record. |
| `challenges/{challengeId}.creatorTemplateOwnerName` | string, optional | Creator display name from the published template record. |
...
- Admin profiles can save and review Pro Trial Review Records under `proTrialReviews` from first-party Pro Trial Interest demand without starting trials, creating purchases, writing entitlements, or changing paid access.
...
- Admin profiles show a Launch Experiment Review Decision Reply Kit with copy-ready approved/waiting/not-ready/declined replies for manual `launchExperimentReviews` decisions without attribution records, tracking pixels, auto-posting, scraped DMs, purchases, entitlements, paid-access claims, outcome promises, or pressure.
...
- Admin profiles can save and review Community Event Review Records under `communityEventReviews` from first-party Community Event Interest demand without creating tickets, orders, payments, merch promises, venue bookings, partner links, payouts, or entitlements.
- Admin profiles show a Community Event Review Decision Reply Kit with copy-ready approved/waiting/not-ready/declined manual replies for community event review records without creating tickets, orders, payments, merch promises, venue bookings, partner links, payouts, entitlements, tracking, auto-messaging, or pressure.
...
- Admin profiles can save and review Customer Value Review Records under `customerValueReviews` from first-party free-loop value signals without charging users, unlocking paid access, creating discounts, writing entitlements, or promoting paid features as live.
- Admin profiles show a Customer Value Review Decision Reply Kit with copy-ready approved/waiting/not-ready/declined manual replies for customer value review records without charging users, unlocking paid access, discounts, purchases, entitlement writes, refunds, marketplace bypass, paid-live promotion, tracking, auto-messaging, or pressure.
...
- Admin profiles show a Store Review Response Kit that exports reviewer follow-up and rejection reply checklists for demo access, permissions, privacy/data safety, purchase evidence, restore behavior, support links, and rejection fixes.
- Admin profiles can save and review Store Review Response Review Records under `storeReviewResponseReviews` from store-test evidence, support coverage, policy links, and product count without submitting store review, exposing private data, including secrets, unlocking paid access, writing entitlements, creating purchases, processing refunds, claiming review approval, marking paid access live, tracking, scraping messages, or bypassing marketplace policy.
- Admin profiles can save and review Paid Launch Decision Review Records under `paidLaunchDecisionReviews` from product count, store-test evidence, support coverage, recovery coverage, and ready-check counts without flipping paid access live, writing entitlements, creating purchases, processing payments/refunds, cancelling subscriptions, collecting payment details, submitting store review, claiming launch readiness, claiming sandbox proof, tracking, scraping messages, or bypassing marketplace policy.
...
- Admin and enabled creator profiles show a Weekly Campaign Operator Summary that turns aggregate reach, active campaign count, referral joins, Feature Me queue size, and the recommended experiment into manual next steps.
- Admin and enabled creator profiles can save manual Weekly Campaign Review Records under `weeklyCampaignReviews` from first-party campaign reach, referral joins, Feature Me queue size, support risk, and recommended experiment context; admin queues can mark records approved/waiting/not ready/declined without attribution records, tracking pixels, auto-posting, scraped DMs, purchases, entitlements, or paid-access changes.
- Admin and enabled creator profiles show a Weekly Campaign Review Decision Reply Kit with copy-ready approved/waiting/not-ready/declined replies for manual `weeklyCampaignReviews` decisions without attribution records, tracking pixels, auto-posting, scraped DMs, purchases, entitlements, or paid-access claims.
...
- Creator / Coach Mode surfaces include a review-only Creator Template Draft Record action that writes `creatorChallengeTemplateDrafts` using creator profile and hosted challenge signals for manual admin review.
- Admin profiles can publish approved creator draft material into a free-first `creatorChallengeTemplates` catalog and show the published catalog on Web, iOS, and Android.
- Challenge creation surfaces on Web, iOS, and Android read published free-first `creatorChallengeTemplates`, show them in the create picker as reviewed creator templates, and stamp created challenges with creator template source metadata.
- Creator / Coach Mode surfaces include a Creator Leaderboard Preview Kit that exports a first-party hosted challenge ranking readiness checklist before any public creator leaderboard records exist.
- Creator / Coach Mode surfaces include a review-only Creator Leaderboard Snapshot action that writes aggregate-only `creatorLeaderboardSnapshots` for manual admin review and published identity-safe aggregate display on Web, iOS, and Android.
...
- Creator / Coach Mode surfaces include a review-only Creator Branded Page Draft action that writes `creatorBrandedPages` with profile, CTA, and hosted challenge summary fields for manual admin review and published app-first Coach Host display on Web, iOS, and Android.
...
- Creator / Coach Mode surfaces include a review-only Private Creator Invite Launch action that writes `creatorPrivateInviteLaunches` with private challenge readiness, invite challenge summary, and manual admin approval state on Web, iOS, and Android.
...
- Creator / Coach Mode surfaces include a Creator Payout Provider Setup Kit that generates a copy-only provider comparison, legal entity, tax workflow, support owner, refund handoff, marketplace boundary, and finance reconciliation checklist before payout onboarding.
- Creator / Coach Mode surfaces include a Creator Payout Operations Dry-Run Kit that generates a copy-only zero-money payout rehearsal checklist for mock eligibility, mock payout events, refund adjustments, finance reconciliation, and owner handoffs before real payout onboarding.
- Creator / Coach Mode surfaces include a Creator Payout Reconciliation Kit that generates a copy-only evidence-matching checklist for mock payout events, store-test evidence, refund adjustments, entitlement QA, finance notes, and owner sign-off before any real payout run.
- Creator / Coach Mode surfaces include a Creator Tax Workflow Readiness Kit that generates a copy-only tax workflow owner, provider tax-form path, country coverage, withholding review, finance export, and approved support-language checklist before creator tax onboarding.
- Creator / Coach Mode surfaces include a Creator Payout Exception Response Kit that generates copy-only manual exception reply checks for store-test mismatches, refund disputes, entitlement gaps, tax workflow gaps, provider setup gaps, support escalation, and creator eligibility questions before payout processing.
- Creator / Coach Mode surfaces include a Creator Payout Support Escalation Kit that generates copy-only support owner, severity, approved response language, privacy, and routing checks before creator payout support is live.
- Creator / Coach Mode surfaces include a Creator Identity Verification Prep Kit that generates a copy-only creator identity, content ownership, brand permission, support owner, moderation owner, and claim-safety checklist before creator terms, payout operations, or revenue-share.
- Creator / Coach Mode surfaces include a Creator Moderation Readiness Kit that generates a copy-only creator conduct, member safety, takedown, support handoff, privacy, escalation, and claim-review checklist before paid-hosting review.
- Creator / Coach Mode surfaces include a Creator Paid Hosting Launch Gate Kit that generates a copy-only go/no-go checklist for profile, agreement, identity, moderation, support, payout, marketplace, store-test evidence, and entitlement QA before paid hosting is promoted.
- Creator / Coach Mode surfaces include a review-only Creator Paid Hosting Launch Gate Evidence action that writes `creatorPaidHostingLaunchGateReviews` with first-party readiness signals while keeping paid hosting off on Web, iOS, and Android.
- Creator / Coach Mode surfaces include a Creator Paid Hosting Hold Plan Kit that generates copy-only hold triggers and response copy when launch-gate evidence, store QA, payout/tax, moderation, support, or marketplace checks are not ready.
...
- Admin profiles show a Partner Campaign Retrospective Kit that exports aggregate-only post-pilot prompts for first-party app movement, partner fit, support notes, and repeat-or-pause decisions without partner links, tracking, payouts, purchases, entitlements, revenue-share, third-party data exports, fulfillment promises, auto-messaging, scraped DMs, or pressure.
...
- Pro Trial Review Records are manual admin evidence only across Web, iOS, and Android. They save to `proTrialReviews` with `manualReviewOnly: true`, `startsTrial: false`, `createsPurchase: false`, `writesEntitlements: false`, and `isPaidAccessLive: false`; review decisions do not start trials, create purchases, grant Pro, write entitlements, or change paid access.
...
- Creator Template Draft Record is review-only/manual UI and must not publish templates, create public template catalogs, create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, add tracking pixels, export private member activity, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
- Published Creator Template Catalog records are admin-reviewed and free-first. They must not create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, add tracking pixels, export private member activity, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
- Published Creator Template create-picker ingestion is free-first and reviewed-only. It must not create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, add tracking pixels, export private member activity, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
- Creator Leaderboard Preview Kit is copy-only/manual planning UI and must not create leaderboard records, expose private member activity, export per-user logs, create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, add tracking pixels, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
- Creator Leaderboard Snapshot records are review-only aggregate hosted-challenge summaries. They must not expose member identities, export per-user logs, create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, add tracking pixels, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
...
- Creator Branded Page records are review-only app-first Coach Host drafts. They must not add tracking pixels, export private member activity, create contracts, collect payments, create purchases, write entitlements, start revenue-share, create payouts, create partner links, scrape/store DMs, store inbound replies, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure creators or members.
...
- Private Creator Invite Launch records are review-only app-first private challenge readiness summaries. They must not auto-message, scrape/store DMs, store inbound replies, count link opens, track recipients, export private member activity, create contracts, create payouts, start revenue-share, collect payments, create purchases, write entitlements, grant paid access, promise earnings, imply paid hosting is live, promise outcomes, imply medical results, or pressure invitees.
...
- Launch Experiment Review records are manual admin evidence only across Web, iOS, and Android. They save to `launchExperimentReviews` with `manualReviewOnly: true`, `createsAttribution: false`, and `isPaidAccessLive: false`, and review decisions do not create attribution records, tracking pixels, purchases, entitlements, or paid-access changes.
- Launch Experiment Review Decision Reply Kit is copy-only/manual UI and must not create attribution records, add tracking pixels, auto-post, scrape/store DMs or replies, create purchases, write entitlements, imply paid access is live, promise outcomes, imply medical results, or pressure members.
...
- Community Event Review Records are manual admin evidence only across Web, iOS, and Android. They save to `communityEventReviews` with `manualReviewOnly: true`, `createsTickets: false`, `createsOrders: false`, `collectsPayments: false`, `booksVenues: false`, `promisesMerch: false`, `createsPartnerLinks: false`, `createsPayouts: false`, and `writesEntitlements: false`; review decisions do not create tickets, orders, payments, merch promises, venue bookings, partner links, payouts, or entitlements.
...
- Customer Value Review Records are manual admin evidence only across Web, iOS, and Android. They save to `customerValueReviews` with `manualReviewOnly: true`, `chargesUsers: false`, `unlocksPaidAccess: false`, `writesEntitlements: false`, `createsDiscounts: false`, `isPaidAccessLive: false`, and `promotesPaidFeatures: false`; review decisions do not charge users, unlock paid access, create discounts, write entitlements, or promote paid features as live.
...
- Support Refund Readiness Review Records are manual admin evidence only across Web, iOS, and Android. They save to `supportRefundReadinessReviews` with `manualReviewOnly: true`, `processesRefunds: false`, `cancelsSubscriptions: false`, `writesEntitlements: false`, `createsPurchases: false`, `bypassesMarketplacePolicy: false`, `collectsPaymentDetails: false`, and `isPaidAccessLive: false`; review decisions do not process refunds, cancel subscriptions, write entitlements, create purchases, collect payment details, bypass marketplace policy, or promote paid access as live.
...
- Store Review Response Kit is copy-only/manual planning UI and must not submit store review, bypass marketplace policy, expose private user data, include passwords or secret keys, unlock paid access, write entitlements, create purchases, process refunds, claim review approval, mark paid access live, promise outcomes, imply medical results, add tracking pixels, scrape DMs, or pressure reviewers or members.
- Store Review Response Review Records are manual admin evidence only across Web, iOS, and Android. They save to `storeReviewResponseReviews` with `manualReviewOnly: true`, `submitsStoreReview: false`, `bypassesMarketplacePolicy: false`, `exposesPrivateUserData: false`, `includesSecrets: false`, `unlocksPaidAccess: false`, `writesEntitlements: false`, `createsPurchases: false`, `processesRefunds: false`, `claimsReviewApproval: false`, `isPaidAccessLive: false`, `hasTrackingPixels: false`, and `scrapesMessages: false`; review decisions do not submit store review, expose private data, include secrets, unlock paid access, write entitlements, create purchases, process refunds, claim review approval, mark paid access live, track, scrape messages, or bypass marketplace policy.
- Paid Launch Decision Review Records are manual admin evidence only across Web, iOS, and Android. They save to `paidLaunchDecisionReviews` with `manualReviewOnly: true`, `flipsPaidAccessLive: false`, `writesEntitlements: false`, `createsPurchases: false`, `processesPayments: false`, `processesRefunds: false`, `cancelsSubscriptions: false`, `collectsPaymentDetails: false`, `submitsStoreReview: false`, `bypassesMarketplacePolicy: false`, `claimsLaunchReadiness: false`, `claimsSandboxProof: false`, `isPaidAccessLive: false`, `hasTrackingPixels: false`, and `scrapesMessages: false`; review decisions do not flip paid access live, write entitlements, create purchases, process payments/refunds, cancel subscriptions, collect payment details, submit store review, claim launch readiness, claim sandbox proof, track, scrape messages, or bypass marketplace policy.
...
- Admin profiles can save and review Partner Perk Handoff Audit Review Records under `partnerPerkHandoffAuditReviews` from aggregate claim, demand, reach, and support-risk evidence without creating coupons, partner links, payouts, discounts, purchases, entitlements, affiliate rewards, tracking pixels, ad targeting, third-party data exports, paid-access claims, payment collection, refunds, fulfillment promises, auto-messages, scraped messages, or pressure.
- Partner Perk Handoff Audit Review Records are manual admin evidence only across Web, iOS, and Android. They save to `partnerPerkHandoffAuditReviews` with `manualReviewOnly: true`, `createsCoupons: false`, `createsPartnerLinks: false`, `createsPayouts: false`, `createsDiscounts: false`, `createsPurchases: false`, `writesEntitlements: false`, `createsAffiliateRewards: false`, `hasTrackingPixels: false`, `usesAdTargeting: false`, `exportsThirdPartyData: false`, `makesPaidAccessClaims: false`, `collectsPayment: false`, `processesRefunds: false`, `promisesFulfillment: false`, `autoMessagesUsers: false`, `scrapesMessages: false`, and `pressuresMembers: false`; review decisions do not create coupons, links, payouts, discounts, purchases, entitlements, tracking, refunds, fulfillment promises, auto-messages, scraped messages, or pressure.
- Admin profiles show `PARTNER PERK HANDOFF AUDIT DECISION REPLY KIT` with `COPY PERK AUDIT DECISION REPLIES`, backed by `partnerPerkHandoffAuditDecisionReplyCopy`, for manual approved/waiting/not-ready/declined audit handoff replies before any partner fulfillment exists.
- Partner Perk Handoff Audit Decision Reply Kit is copy-only/manual UI and must not create coupons, partner links, payouts, discounts, purchases, entitlements, affiliate rewards, tracking pixels, ad targeting, third-party data exports, paid-access claims, collect payment, process refunds, promise fulfillment, promise outcomes, imply medical results, auto-message users, scrape/store DMs, or pressure members.
- Admin profiles show `PARTNER PERK SUPPORT ESCALATION KIT` with `COPY PERK SUPPORT KIT`, backed by `partnerPerkSupportEscalationCopy`, for support owner, severity, response-language, privacy, and routing checks before partner perk support or fulfillment exists.
- Partner Perk Support Escalation Kit is copy-only/manual UI and must not provide live perk fulfillment, create coupons, create partner links, process refunds, collect payment details, create payouts, create discounts, create purchases, write entitlements, create affiliate rewards, add tracking pixels, use ad targeting, export third-party data, expose private member logs, scrape/store DMs, auto-message users, promise fulfillment, imply paid access is live, promise outcomes, imply medical results, or pressure members.
...
- Admin profiles show a Partner Campaign Retrospective Kit that turns first-party partner demand, campaign reach, referral joins, open partner applications, and open perk claims into aggregate-only post-pilot prompts before repeating or pausing sponsor outreach.
...
- Admin profile surfaces include a Store Screenshot QA Kit that exports reviewer-safe screenshot and caption prompts from seeded demo data, policy links, store-test evidence, and paid-launch gate status without private data, credentials, purchase tokens, paid-live claims, or store-submission side effects.
...
| Referral Reward Handoff Audit Review Records | Manual `referralRewardHandoffAuditReviews` admin evidence queue with no Pro grant, entitlement, challenge-pack unlock, payout, purchase, discount, affiliate reward, referral-state write, link-open count, fulfillment claim, tracking, auto-message, scraped/stored reply, or pressure side effects | Manual `referralRewardHandoffAuditReviews` admin evidence queue with no Pro grant, entitlement, challenge-pack unlock, payout, purchase, discount, affiliate reward, referral-state write, link-open count, fulfillment claim, tracking, auto-message, scraped/stored reply, or pressure side effects | Manual `referralRewardHandoffAuditReviews` admin evidence queue with no Pro grant, entitlement, challenge-pack unlock, payout, purchase, discount, affiliate reward, referral-state write, link-open count, fulfillment claim, tracking, auto-message, scraped/stored reply, or pressure side effects |
| Referral Reward Handoff Audit Decision Reply Kit | Admin-only copy-ready approved/waiting/not-ready/declined referral audit replies before Pro grants, entitlements, challenge-pack unlocks, discounts, payouts, purchases, affiliate rewards, referral-state writes, tracking, or fulfillment claims exist | Admin-only copy-ready approved/waiting/not-ready/declined referral audit replies before Pro grants, entitlements, challenge-pack unlocks, discounts, payouts, purchases, affiliate rewards, referral-state writes, tracking, or fulfillment claims exist | Admin-only copy-ready approved/waiting/not-ready/declined referral audit replies before Pro grants, entitlements, challenge-pack unlocks, discounts, payouts, purchases, affiliate rewards, referral-state writes, tracking, or fulfillment claims exist |
...
| Private Creator Invite Launch Review | Review-only `creatorPrivateInviteLaunches` private challenge readiness records with manual approval | Review-only `creatorPrivateInviteLaunches` private challenge readiness records with manual approval | Review-only `creatorPrivateInviteLaunches` private challenge readiness records with manual approval |
...
| Creator Payout Provider Setup Kit | Pro Coach Mode provider comparison, legal entity, tax workflow, support, refund handoff, marketplace boundary, and finance reconciliation checklist, copy-only | Pro Coach Mode provider comparison, legal entity, tax workflow, support, refund handoff, marketplace boundary, and finance reconciliation checklist, copy-only | Pro Coach Mode provider comparison, legal entity, tax workflow, support, refund handoff, marketplace boundary, and finance reconciliation checklist, copy-only |
| Creator Payout Operations Dry-Run Kit | Pro Coach Mode zero-money payout rehearsal for mock eligibility, payout event, refund adjustment, finance reconciliation, and owner handoffs, copy-only | Pro Coach Mode zero-money payout rehearsal for mock eligibility, payout event, refund adjustment, finance reconciliation, and owner handoffs, copy-only | Pro Coach Mode zero-money payout rehearsal for mock eligibility, payout event, refund adjustment, finance reconciliation, and owner handoffs, copy-only |
| Creator Payout Reconciliation Kit | Pro Coach Mode mock payout evidence matching for store-test evidence, refund adjustments, entitlement QA, finance notes, and owner sign-off, copy-only | Pro Coach Mode mock payout evidence matching for store-test evidence, refund adjustments, entitlement QA, finance notes, and owner sign-off, copy-only | Pro Coach Mode mock payout evidence matching for store-test evidence, refund adjustments, entitlement QA, finance notes, and owner sign-off, copy-only |
| Creator Tax Workflow Readiness Kit | Pro Coach Mode tax workflow owner, provider tax-form path, country coverage, withholding review, finance export, and approved-support-language checklist, copy-only | Pro Coach Mode tax workflow owner, provider tax-form path, country coverage, withholding review, finance export, and approved-support-language checklist, copy-only | Pro Coach Mode tax workflow owner, provider tax-form path, country coverage, withholding review, finance export, and approved-support-language checklist, copy-only |
| Creator Payout Exception Response Kit | Pro Coach Mode manual exception replies for store-test mismatches, refund disputes, entitlement gaps, tax workflow gaps, provider setup gaps, support escalation, and creator eligibility, copy-only | Pro Coach Mode manual exception replies for store-test mismatches, refund disputes, entitlement gaps, tax workflow gaps, provider setup gaps, support escalation, and creator eligibility, copy-only | Pro Coach Mode manual exception replies for store-test mismatches, refund disputes, entitlement gaps, tax workflow gaps, provider setup gaps, support escalation, and creator eligibility, copy-only |
| Creator Payout Support Escalation Kit | Pro Coach Mode payout support owner, severity, approved response language, privacy, and routing checks, copy-only | Pro Coach Mode payout support owner, severity, approved response language, privacy, and routing checks, copy-only | Pro Coach Mode payout support owner, severity, approved response language, privacy, and routing checks, copy-only |
| Creator Identity Verification Prep Kit | Pro Coach Mode identity, content ownership, brand permission, support, moderation, and claim-safety checklist, copy-only | Pro Coach Mode identity, content ownership, brand permission, support, moderation, and claim-safety checklist, copy-only | Pro Coach Mode identity, content ownership, brand permission, support, moderation, and claim-safety checklist, copy-only |
| Creator Moderation Readiness Kit | Pro Coach Mode creator conduct, member safety, takedown, support handoff, privacy, escalation, and claim-review checklist, copy-only | Pro Coach Mode creator conduct, member safety, takedown, support handoff, privacy, escalation, and claim-review checklist, copy-only | Pro Coach Mode creator conduct, member safety, takedown, support handoff, privacy, escalation, and claim-review checklist, copy-only |
| Creator Paid Hosting Launch Gate Kit | Pro Coach Mode final profile, agreement, identity, moderation, support, payout, marketplace, store-test, and entitlement QA gate, copy-only | Pro Coach Mode final profile, agreement, identity, moderation, support, payout, marketplace, store-test, and entitlement QA gate, copy-only | Pro Coach Mode final profile, agreement, identity, moderation, support, payout, marketplace, store-test, and entitlement QA gate, copy-only |
| Creator Paid Hosting Launch Gate Evidence Review | Review-only `creatorPaidHostingLaunchGateReviews` first-party readiness records, paid hosting remains off | Review-only `creatorPaidHostingLaunchGateReviews` first-party readiness records, paid hosting remains off | Review-only `creatorPaidHostingLaunchGateReviews` first-party readiness records, paid hosting remains off |
| Creator Paid Hosting Hold Plan Kit | Pro Coach Mode hold triggers and response copy when launch-gate evidence, store QA, payout/tax, moderation, support, or marketplace checks are not ready, copy-only | Pro Coach Mode hold triggers and response copy when launch-gate evidence, store QA, payout/tax, moderation, support, or marketplace checks are not ready, copy-only | Pro Coach Mode hold triggers and response copy when launch-gate evidence, store QA, payout/tax, moderation, support, or marketplace checks are not ready, copy-only |
...
| Customer Value Review Records | Manual `customerValueReviews` admin evidence queue with no charge, paid-access unlock, discount, entitlement, or paid-live side effects | Manual `customerValueReviews` admin evidence queue with no charge, paid-access unlock, discount, entitlement, or paid-live side effects | Manual `customerValueReviews` admin evidence queue with no charge, paid-access unlock, discount, entitlement, or paid-live side effects |
| Customer Value Review Decision Reply Kit | Admin-only copy-ready approved/waiting/not-ready/declined replies without charges, paid-access unlocks, discounts, purchases, entitlement writes, refunds, marketplace bypass, paid-live promotion, tracking, auto-messaging, or pressure | Admin-only copy-ready approved/waiting/not-ready/declined replies without charges, paid-access unlocks, discounts, purchases, entitlement writes, refunds, marketplace bypass, paid-live promotion, tracking, auto-messaging, or pressure | Admin-only copy-ready approved/waiting/not-ready/declined replies without charges, paid-access unlocks, discounts, purchases, entitlement writes, refunds, marketplace bypass, paid-live promotion, tracking, auto-messaging, or pressure |
...
| Support Refund Readiness Review Records | Manual `supportRefundReadinessReviews` admin evidence queue with no refund, cancellation, purchase, entitlement, payment-detail, marketplace-bypass, or paid-live side effects | Manual `supportRefundReadinessReviews` admin evidence queue with no refund, cancellation, purchase, entitlement, payment-detail, marketplace-bypass, or paid-live side effects | Manual `supportRefundReadinessReviews` admin evidence queue with no refund, cancellation, purchase, entitlement, payment-detail, marketplace-bypass, or paid-live side effects |
...
| Store Screenshot QA Kit | Admin copy-only reviewer-safe screenshot and caption prompts from seeded demo data, policy links, store evidence, and paid-launch gate status | Admin copy-only reviewer-safe screenshot and caption prompts from seeded demo data, policy links, store evidence, and paid-launch gate status | Admin copy-only reviewer-safe screenshot and caption prompts from seeded demo data, policy links, store evidence, and paid-launch gate status |
...
| Store Review Response Kit | Admin reviewer follow-up and rejection reply checklist for demo, permissions, privacy/data safety, purchase evidence, restore, support, and fixes | Admin reviewer follow-up and rejection reply checklist for demo, permissions, privacy/data safety, purchase evidence, restore, support, and fixes | Admin reviewer follow-up and rejection reply checklist for demo, permissions, privacy/data safety, purchase evidence, restore, support, and fixes |
| Store Review Response Review Records | Manual `storeReviewResponseReviews` admin evidence queue with no store submission, private-data exposure, secret sharing, paid unlock, entitlement, purchase, refund, review-approval claim, paid-live, tracking, message-scraping, or marketplace-bypass side effects | Manual `storeReviewResponseReviews` admin evidence queue with no store submission, private-data exposure, secret sharing, paid unlock, entitlement, purchase, refund, review-approval claim, paid-live, tracking, message-scraping, or marketplace-bypass side effects | Manual `storeReviewResponseReviews` admin evidence queue with no store submission, private-data exposure, secret sharing, paid unlock, entitlement, purchase, refund, review-approval claim, paid-live, tracking, message-scraping, or marketplace-bypass side effects |
| Paid Launch Decision Review Records | Manual `paidLaunchDecisionReviews` admin evidence queue with no paid-live flip, entitlement, purchase, payment, refund, cancellation, payment-detail, store-submission, launch-readiness, sandbox-proof, tracking, message-scraping, or marketplace-bypass side effects | Manual `paidLaunchDecisionReviews` admin evidence queue with no paid-live flip, entitlement, purchase, payment, refund, cancellation, payment-detail, store-submission, launch-readiness, sandbox-proof, tracking, message-scraping, or marketplace-bypass side effects | Manual `paidLaunchDecisionReviews` admin evidence queue with no paid-live flip, entitlement, purchase, payment, refund, cancellation, payment-detail, store-submission, launch-readiness, sandbox-proof, tracking, message-scraping, or marketplace-bypass side effects |
...
| Launch Experiment Review Records | Manual `launchExperimentReviews` admin evidence queue with no attribution, tracking, purchase, entitlement, or paid-access side effects | Manual `launchExperimentReviews` admin evidence queue with no attribution, tracking, purchase, entitlement, or paid-access side effects | Manual `launchExperimentReviews` admin evidence queue with no attribution, tracking, purchase, entitlement, or paid-access side effects |
| Launch Experiment Review Decision Reply Kit | Admin-only copy-ready approved/waiting/not-ready/declined launch experiment replies without attribution, tracking, purchase, entitlement, paid-access, automation, or pressure side effects | Admin-only copy-ready approved/waiting/not-ready/declined launch experiment replies without attribution, tracking, purchase, entitlement, paid-access, automation, or pressure side effects | Admin-only copy-ready approved/waiting/not-ready/declined launch experiment replies without attribution, tracking, purchase, entitlement, paid-access, automation, or pressure side effects |
| Pro Trial Review Records | Manual `proTrialReviews` admin evidence queue with no trial, purchase, entitlement, Pro grant, or paid-access side effects | Manual `proTrialReviews` admin evidence queue with no trial, purchase, entitlement, Pro grant, or paid-access side effects | Manual `proTrialReviews` admin evidence queue with no trial, purchase, entitlement, Pro grant, or paid-access side effects |
| Community Event Review Records | Manual `communityEventReviews` admin evidence queue with no ticket, order, payment, merch promise, venue, partner-link, payout, or entitlement side effects | Manual `communityEventReviews` admin evidence queue with no ticket, order, payment, merch promise, venue, partner-link, payout, or entitlement side effects | Manual `communityEventReviews` admin evidence queue with no ticket, order, payment, merch promise, venue, partner-link, payout, or entitlement side effects |
| Community Event Review Decision Reply Kit | Admin-only copy-ready approved/waiting/not-ready/declined replies without tickets, orders, payments, merch promises, venues, partner links, payouts, entitlements, tracking, auto-messaging, or pressure | Admin-only copy-ready approved/waiting/not-ready/declined replies without tickets, orders, payments, merch promises, venues, partner links, payouts, entitlements, tracking, auto-messaging, or pressure | Admin-only copy-ready approved/waiting/not-ready/declined replies without tickets, orders, payments, merch promises, venues, partner links, payouts, entitlements, tracking, auto-messaging, or pressure |
...
| Weekly Campaign Operator Summary | Admin/creator aggregate reach, active campaign, referral, Feature Me queue, experiment lens, and next manual action readout | Admin/creator aggregate reach, active campaign, referral, Feature Me queue, experiment lens, and next manual action readout | Admin/creator aggregate reach, active campaign, referral, Feature Me queue, experiment lens, and next manual action readout |
...
| Weekly Campaign Review Records | Manual `weeklyCampaignReviews` admin evidence queue with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access side effects | Manual `weeklyCampaignReviews` admin evidence queue with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access side effects | Manual `weeklyCampaignReviews` admin evidence queue with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access side effects |
| Weekly Campaign Review Decision Reply Kit | Admin copy-ready approved/waiting/not-ready/declined replies for manual weekly campaign review decisions with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access claims | Admin copy-ready approved/waiting/not-ready/declined replies for manual weekly campaign review decisions with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access claims | Admin copy-ready approved/waiting/not-ready/declined replies for manual weekly campaign review decisions with no attribution, tracking pixel, auto-posting, scraped-DM, purchase, entitlement, or paid-access claims |
...
| Partner Perk Handoff Audit Review Records | Manual `partnerPerkHandoffAuditReviews` admin evidence queue with no coupon, link, payout, discount, purchase, entitlement, affiliate reward, tracking, ad-targeting, third-party export, paid-access claim, payment, refund, fulfillment promise, auto-message, scraped-message, or pressure side effects | Manual `partnerPerkHandoffAuditReviews` admin evidence queue with no coupon, link, payout, discount, purchase, entitlement, affiliate reward, tracking, ad-targeting, third-party export, paid-access claim, payment, refund, fulfillment promise, auto-message, scraped-message, or pressure side effects | Manual `partnerPerkHandoffAuditReviews` admin evidence queue with no coupon, link, payout, discount, purchase, entitlement, affiliate reward, tracking, ad-targeting, third-party export, paid-access claim, payment, refund, fulfillment promise, auto-message, scraped-message, or pressure side effects |
| Partner Perk Handoff Audit Decision Reply Kit | Admin-only copy-ready approved/waiting/not-ready/declined audit handoff replies before coupons, partner links, payouts, discounts, purchases, entitlements, tracking, refunds, or fulfillment promises exist | Admin-only copy-ready approved/waiting/not-ready/declined audit handoff replies before coupons, partner links, payouts, discounts, purchases, entitlements, tracking, refunds, or fulfillment promises exist | Admin-only copy-ready approved/waiting/not-ready/declined audit handoff replies before coupons, partner links, payouts, discounts, purchases, entitlements, tracking, refunds, or fulfillment promises exist |
| Partner Perk Support Escalation Kit | Admin-only copy-ready support owner, severity, approved response-language, privacy, and routing checks before coupons, links, refunds, purchases, entitlements, or fulfillment exist | Admin-only copy-ready support owner, severity, approved response-language, privacy, and routing checks before coupons, links, refunds, purchases, entitlements, or fulfillment exist | Admin-only copy-ready support owner, severity, approved response-language, privacy, and routing checks before coupons, links, refunds, purchases, entitlements, or fulfillment exist |
...
| Partner Campaign Retrospective Kit | Admin-only aggregate post-pilot readback for app movement, partner fit, support notes, and repeat-or-pause decisions before links, tracking, payouts, purchases, entitlements, revenue-share, or fulfillment promises exist | Admin-only aggregate post-pilot readback for app movement, partner fit, support notes, and repeat-or-pause decisions before links, tracking, payouts, purchases, entitlements, revenue-share, or fulfillment promises exist | Admin-only aggregate post-pilot readback for app movement, partner fit, support notes, and repeat-or-pause decisions before links, tracking, payouts, purchases, entitlements, revenue-share, or fulfillment promises exist |
...
| Store Screenshot QA Kit | Admin-only reviewer-safe screenshot/caption QA prompts, copy-only | Admin-only reviewer-safe screenshot/caption QA prompts, copy-only | Admin-only reviewer-safe screenshot/caption QA prompts, copy-only |
