export function buildWeeklyCampaignCollabCopyCards({
  ...
  const weeklyCampaignSupportTriageCopy = `Rise With The Tribe Weekly Campaign Support Triage Kit:\n\nCampaign reviewed: ${weeklyCampaignPrompt.name}\nWeek: ${weeklyCampaignPrompt.week} (${weeklyCampaignPrompt.label})\nHashtag: ${weeklyCampaignPrompt.hashtag}\nOpen support-risk items: ${supportReviewQueue.length}\nReferral joins: ${referralJoins}\nPending feature submissions: ${featureReviewQueue.length}\nRecommended experiment: ${recommendedLaunchExperiment.label}\n\nManual triage lanes:\n[ ] Billing or refund concern: route to Support and Refund Readiness, Billing Support Escalation, or Entitlement Recovery copy before any campaign follow-up.\n[ ] Privacy, deletion, or account concern: use support decision copy and avoid campaign prompts until the member issue is resolved.\n[ ] Paid-access confusion: repeat that Pro, challenge packs, creator hosting, and partner perks are not live until external store evidence and review gates are complete.\n[ ] UGC or Feature Me concern: confirm consent and review status before any public repost, recap, or creator collab mention.\n[ ] Safe re-invite candidate: only after support risk is resolved, invite one app-first log or accountability partner without discounts, pressure, or paid claims.\n\nOperator note:\nUse this before retention or re-invite copy when campaign replies include support risk. Keep member context inside first-party support review; Instagram reactions can guide support themes but should not become stored identity, attribution, or product-decision evidence.\n\nThis is a manual weekly campaign support triage kit only. Do not auto-message users, scrape posts, scrape comments, scrape DMs, store Instagram identities, store inbound replies, create support tickets outside the app, process refunds, cancel subscriptions, collect payment details, create purchases, write entitlements, unlock paid access, expose private support notes, imply paid access is live, promise outcomes, imply medical results, or pressure members.`;
  const weeklyCampaignSupportReadinessScriptCopy = `Rise With The Tribe Weekly Campaign Support Readiness Script Kit:\n\nCampaign reviewed: ${weeklyCampaignPrompt.name}\nWeek: ${weeklyCampaignPrompt.week} (${weeklyCampaignPrompt.label})\nHashtag: ${weeklyCampaignPrompt.hashtag}\nOpen support-risk items: ${supportReviewQueue.length}\nReferral joins: ${referralJoins}\nPending feature submissions: ${featureReviewQueue.length}\nRecommended experiment: ${recommendedLaunchExperiment.label}\n\nManual support readiness script:\n1. Confirm the member's campaign question is resolved before using retention, re-invite, collab, Feature Me, Pro trial, challenge pack, creator hosting, or partner perk copy.\n2. Capture only first-party support context: app account, support category, review status, consent state, restore/sync result if relevant, and the safest in-app next action.\n3. Route billing/refund, privacy/deletion, paid-access confusion, UGC consent, and safety concerns to the matching in-app support or review queue before any campaign follow-up.\n4. Use a free-loop next action when support is unresolved: log one honest session, wait for support review, update consent, or open the app support request.\n\nThis support readiness script is copy-only. Do not auto-message users, scrape posts, scrape comments, scrape DMs, store Instagram identities, create support tickets outside the app, process refunds, cancel subscriptions, collect payment details, create purchases, write entitlements, unlock paid access, expose private support notes, imply paid access is live, promise outcomes, imply medical results, or pressure members.`;
  ...
  const weeklyCampaignCollabCopyCards = [
    ...
    {
      title: 'Weekly Campaign Support Triage Kit',
      subtitle: 'Manual support-risk triage before re-invites',
      status: 'SUPPORT',
      accent: '#F97316',
      body: 'Copy support-risk triage lanes for billing, privacy, paid-access confusion, and Feature Me concerns before retention or re-invite prompts.',
      buttonLabel: 'COPY SUPPORT TRIAGE',
      copyText: weeklyCampaignSupportTriageCopy,
    },
    {
      title: 'Weekly Campaign Support Readiness Script Kit',
      subtitle: 'Member-safe support follow-up before campaign prompts',
      status: 'SUPPORT',
      accent: '#FB923C',
      body: 'Copy a member-safe support readiness script before retention, re-invite, collab, Feature Me, Pro, pack, creator, or partner copy is used.',
      buttonLabel: 'COPY SUPPORT READINESS SCRIPT',
      copyText: weeklyCampaignSupportReadinessScriptCopy,
    },
    {
